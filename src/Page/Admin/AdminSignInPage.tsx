...
import styles from './admin-signin-page.module.scss';
import { AdminSignInInput } from '../../types';

const defaultInput: AdminSignInInput = {
  email: '',
  password: ''
};

const SignInPage: React.FC = () => {
  const [itemInput, setItemInput] = useState<AdminSignInInput>(defaultInput);
  const [loading, setLoading] = useState<boolean>();
  const [errorText, setErrorText] = useState<AdminSignInInput>(defaultInput);
  ...
  const onSignIn = async () => {
    ...
    try {
      ...
      if (e instanceof Error) { // type assertion here
        setError(e.message);
      } else {
  ...
  const onItemInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setItemInput({
      ...itemInput,
    ...
    <div className={styles.signInPageRoot}>
      <div className={styles.signInWrapper}>
        ...
        <TextField className={styles.inputField} id="password" name="password" label="Password"
          type="password" value={itemInput.password}
